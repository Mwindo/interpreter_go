...
func TestErrorHandling(t *testing.T) {
	...
	for _, tt := range tests {
		...
		if errOb.Message != tt.expectedMessage {
			t.Errorf("wrong error message. expected=%q, got=%q", tt.expectedMessage, errOb.Message)
		}
	}
}
...
func TestFunctionObject(t *testing.T) {
	...
	if fn.Parameters[0].String() != "x" {
		t.Fatalf("parameter is not 'x'. got='%q' instead", fn.Parameters[0])
	}
...
func TestBuiltInFunctions(t *testing.T) {
	tests := []struct {
		...
		{`len("one", "two");`, "wrong number of arguments to `len`: got 2"},
	}
...
func TestArrayLiterals(t *testing.T) {
	...
	for index, expectedVal := range expected {
		testIntegerObject(t, result.Elements[index], expectedVal)
	}
}
